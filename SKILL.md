...
### 地区覆盖提醒 (Region Coverage Alert)

**当前法规库仅覆盖：浙江省、江苏省、上海市、长三角区域协作。**

When the user's legal matter involves a location outside these regions:
1. Call `statutes.detect_uncovered_regions(text)` to identify un-covered provinces
2. If uncovered regions are found, **DO NOT silently proceed** — remind the user:
   > "当前 skill 仅包含江浙沪地方法规。此案件涉及 [广东/北京/...] 地区，地方法规可能不适用。是否需要我协助添加该地区法规？具体需要哪些法规名称和条文？"
